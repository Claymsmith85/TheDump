...
function Get-SendGridClientFromLocalSupport {
	# Mirrors Jira.ps1's Get-JiraAuthContextFromLocalSupport: independently
	# dot-sources config/LocalVariables.ps1 and Delinea/DelineaAuth.ps1 from the
	# sibling Clayton_M365_Scripts repository so this helper does not depend on
	# what a prior M365Authentication.ps1 call happened to load into its own
	# function scope.
	[CmdletBinding()]
	param(
		[Parameter(Mandatory)]
	...
	$configPath = Join-Path $SupportRoot 'config\LocalVariables.ps1'
	$authPath = Join-Path $SupportRoot 'Delinea\DelineaAuth.ps1'
	. $configPath
	. $authPath

	foreach ($requiredVariable in @('SsBase', 'SendGridSecretId')) {
		$value = Get-Variable -Name $requiredVariable -ValueOnly -ErrorAction SilentlyContinue
		if ([string]::IsNullOrWhiteSpace([string]$value)) {
			throw "Local configuration variable '$requiredVariable' is empty in '$configPath'."
		}
	}

	$ssToken = Get-DelineaToken -SsBase $SsBase -ProbeSecretId $SendGridSecretId -Comment 'SendGrid group-driven teammate sync.'
	return New-SendGridClientFromDelinea -SsBase $SsBase -SecretId $SendGridSecretId -SsToken $ssToken -Region $Region -DefaultPageSize $DefaultPageSize
}
